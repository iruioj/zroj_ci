...
{% for item in added %}- `{{ item }}`
{% endfor %}
{% endif -%}
{% if changed.len() != 0 -%}
### Changed

{% for item in changed %}- Change `{{ item.old }}`
  to `{{ item.new }}`
{% endfor %}
{% endif -%}
{% if removed.len() != 0 -%}
...
impl PublicApiDiff {
    /// render changelog markdown string
    ...
    fn item_with_path(path_str: &str) -> PublicItem {
        new_public_item(
            path_str
                ...
                .collect(),
            vec![super::super::tokens::Token::identifier(path_str)],
        )
    }
